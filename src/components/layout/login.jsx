...
const RegisterPage = () => {
  ...
  const registerUser = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://hard-investing.gl.at.ply.gg:9321/api/users/register", {
        method: "POST",
  ...
  const loginUser = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://hard-investing.gl.at.ply.gg:9321/api/users/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: formData.email,
