...
const RegisterPage = () => {
  ...
  useEffect(() => {
    const eventDetail = async() => {
      try{
        ...
        setTeamConfig({
            minTeamMembers: data.minMembers,
            maxTeamMembers: data.maxMembers,
          });
          if (data.eventName && !eventNameFromState) {
            setDisplayEventName(data.eventName);
          }
        }
      catch(error){
  ...
  try {
    const response = await fetch(`https://avalanche.git.edu/api/user/${avalancheId}`);
    ...
    const newPaymentStatus = [...paymentStatus];
    // NOW without the NOT operator, accessing the correct nested property
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const stats = await fetch(
        `https://avalanche.git.edu/api/events/${eventId}/stats`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          ...
          {formData.teamSize && formData.userIds.map((member, index) => (
            <div key={index} className="relative">
              ...
              <input
                ...
                className={`w-full bg-black/40 rounded-md px-4 py-2 text-white focus:outline-none transition-all duration-300 ${
                  paymentStatus[index] === true
